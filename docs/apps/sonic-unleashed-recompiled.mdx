...
1.  **Download UWP Files:** Get the latest `.msixbundle` and `.appx` dependency from the UWP port's [GitHub releases page](https://github.com/YTReviveMe/UnleashedRecomp-uwp/releases).
2.  **Access Dev Portal:** Connect to your Xbox Device Portal.
3.  **Navigate to "Add":** Go to "My games & apps" > "Add".
4.  **Upload MSIXBUNDLE:** Upload the `.msixbundle` file.
5.  **Upload Dependency:** Click "Next". Upload the `.appx` dependency file.
...
1.  **Connect USB Drive:** Ensure your external USB drive, **now assigned as `E:` on your PC during setup**, is plugged into your Xbox *before* launching.
...
:::caution
This UWP port is by WorleyDL. Report UWP-specific issues to them, **not** Hedge-Dev or HedgeModManager developers.
:::
