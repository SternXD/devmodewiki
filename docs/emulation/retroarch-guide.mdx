...
*   **Xbox Dev Store (Recommended for most users):**
    *   This is the easiest method. See the [Installation](#installation) section below for download instructions.
...
4.  **WARNING:** **DO NOT** change the `Core` directory. This will prevent RetroArch from finding your downloaded cores.
...
4.  **Wait:** RetroArch will scan the folder and attempt to match games to its database, creating a playlist.
    *   **Note:** You *must* have the corresponding core downloaded *before* scanning, otherwise RetroArch may crash.
