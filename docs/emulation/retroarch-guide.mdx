...
<AtAGlance
  ...
  works={[
    'Dev Store builds (Mesa/Angle) install via Device Portal',
...
*   **Xbox Dev Store (Recommended for most users):**
    *   This is the easiest method. See the [Installing RetroArch on Xbox](#3-installing-retroarch-on-xbox) section below for download instructions.
    :::note
    The versions on the Dev Store (excluding the Citra build) typically *do not* come with cores pre-installed. You will need to download them later using the Online Updater (covered in Step 5).
    :::
